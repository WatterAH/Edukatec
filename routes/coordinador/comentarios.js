...
const { isAjax, descifrar } = require("../server/middlewares/functions");
...
app.get("/comentarios", isAjax, async (req, res) => {
  ...
  const id_sala = await hallExists(
    type_report,
    id_pattern,
    req.session.idC,
    maestro
...
app.post("/commentC", isAjax, [body("txt").trim()], async (req, res) => {
  ...
  let id_pattern = await hallExists(type, reportID, req.session.idC, maestro);
  if (!txt) {
